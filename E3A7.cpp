...
int *ptomx(int a[], int n){
	//methodos typou int deiktei
	//alla epistrefei address
    int max = a[0];
    int *pos = &a[0];//address tis adress tou a[0]
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        {
            max = a[i];
            pos = &a[i];//pos = adress tou a[0]
        }
    }

    return pos;//return address
}
//st
